...
@section('title', 'Select Project for PoC - DjokiHub')
...
<div class="bg-white shadow-sm border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="text-center">
            {{-- Icon --}}
            <div class="mx-auto w-20 h-20 bg-cyan-100 rounded-2xl flex items-center justify-center mb-6">
                <i class="ri-file-text-line text-cyan-600 text-3xl"></i>
                        </div>
            ...
            <p class="text-xl text-gray-600 mb-8 max-w-2xl mx-auto">
                Generate comprehensive Proof of Concept documents for your projects
            </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mt-12">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all duration-300">
                    <div class="text-3xl font-bold text-cyan-600 mb-2">{{ $projects->whereIn('status', ['Active', 'On-going', 'Pending'])->count() }}</div>
                    <div class="text-gray-600 text-sm">Available for PoC</div>
                </div>
                <div class="bg-white border border-gray-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all duration-300">
                    <div class="text-3xl font-bold text-blue-600 mb-2">{{ $projects->where('status', 'Draft')->count() }}</div>
                    <div class="text-gray-600 text-sm">Draft Projects</div>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="w-12 h-12 bg-cyan-100 rounded-xl flex items-center justify-center">
                            <i class="ri-file-text-line text-cyan-600 text-xl"></i>
                        </div>
                    <div>
                            <h2 class="text-2xl font-bold text-gray-900">Project Selection for PoC</h2>
                            <p class="text-gray-600 text-sm mt-1">Choose a project to generate its Proof of Concept document</p>
                        </div>
                    </div>
                    <div class="hidden lg:flex items-center space-x-2 bg-gray-50 rounded-xl px-4 py-2">
                        <div class="w-3 h-3 bg-cyan-500 rounded-full"></div>
                        <span class="text-gray-700 text-sm font-medium">Ready for PoC</span>
                    </div>
                </div>
            </div>
                        ...
                        <tr class="hover:bg-gray-50 transition-all duration-300 group">
                            <td class="px-4 py-4">
                                <div class="flex items-center space-x-3">
                                    <div class="w-8 h-8 bg-cyan-100 rounded-lg flex items-center justify-center shadow-sm">
                                        <i class="ri-briefcase-4-line text-cyan-600 text-sm"></i>
                                    </div>
                                    <div>
                                        <div class="text-sm font-bold text-gray-900 group-hover:text-cyan-700 transition-colors">
                                            {{ Str::limit($project->project_name, 30) }}
                                        </div>
                                        <div class="text-xs font-medium text-cyan-600 bg-cyan-100 px-2 py-1 rounded-full inline-block mt-1">
                            ...
                            <td class="px-4 py-4">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    @if(in_array($project->status, ['Active', 'On-going', 'Pending', 'Draft']))
                                        <div class="w-2 h-2 bg-cyan-400 rounded-full" title="Ready for PoC"></div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex justify-center">
                                    <button onclick="showConfirmModal('{{ $project->id }}', '{{ addslashes($project->project_name) }}', 'PoC', '{{ route('projects.poc.generate', $project->id) }}')" 
                                            class="group relative inline-flex items-center px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-medium text-sm rounded-lg shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105">
                                        <i class="ri-file-text-line mr-1 text-sm"></i>
                            ...
                            <td colspan="5" class="px-4 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    <div class="w-16 h-16 bg-cyan-100 rounded-full flex items-center justify-center mb-4">
                                        <i class="ri-folder-open-line text-3xl text-cyan-400"></i>
                                    </div>
                                    <h3 class="text-lg font-bold text-gray-900 mb-2">No Projects Available</h3>
                                    <p class="text-gray-500 mb-4 max-w-md text-sm">There are no projects available for PoC generation at this time.</p>
                                    <a href="{{ route('projects.index') }}" class="inline-flex items-center px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-medium text-sm rounded-lg shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105">
...
<div id="confirmationModal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm hidden flex items-center justify-center z-50 transition-all duration-300" style="opacity: 0;">
    <div class="relative mx-auto p-6 w-full max-w-3xl bg-white rounded-2xl shadow-2xl transform transition-all duration-300 max-h-[90vh] overflow-y-auto" style="transform: scale(0.9);">
        ...
        <div class="text-center mb-6">
            <div id="modalIconContainer" class="mx-auto w-16 h-16 rounded-2xl flex items-center justify-center mb-4 bg-teal-100">
                <i id="modalIcon" class="text-teal-600 text-2xl"></i>
            </div>
            <h3 id="modalTitle" class="text-xl font-bold text-gray-900 mb-2"></h3>
            <p class="text-gray-600 text-sm">
                Please review the project details before generating the <strong id="modalDocTypeStrong" class="text-teal-600"></strong>
            </p>
        </div>
        ...
        <div class="bg-teal-50 rounded-xl p-4 mb-6 border border-teal-200">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 bg-teal-100 rounded-xl flex items-center justify-center">
                    <i class="ri-briefcase-4-line text-teal-600 text-lg"></i>
                </div>
                <div class="flex-1">
                    <div class="font-bold text-gray-900 text-sm" id="modalProjectName"></div>
                    <div class="text-xs text-gray-600">Project selected for PoC generation</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div class="flex items-start space-x-2">
                <i class="ri-alert-line text-blue-600 mt-0.5"></i>
                <div>
                    <h4 class="font-semibold text-blue-800 text-sm">Confirmation Required</h4>
                    <p class="text-blue-700 text-sm mt-1">
                        Please verify that all project information above is correct. The PoC document will be generated 
                        based on this information and will include the project description and requirements listed above.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex space-x-3">
                ...
                <button id="confirmGenerateButton"
                    class="flex-1 px-4 py-2 text-white font-medium text-sm rounded-lg transition-all duration-200 bg-teal-600 hover:bg-teal-700 shadow-lg hover:shadow-xl">
...
<script>
    ...
    function showConfirmModal(projectId, projectName, docType, actionUrl) {
        ...
        if (docType.toLowerCase() === 'poc') {
            modalTitle.textContent = 'Confirm PoC Generation';
            modalIcon.className = 'ri-file-text-line text-teal-600 text-2xl';
        }
